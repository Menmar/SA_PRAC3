...
public class DigitalSessionRepositoryImpl implements DigitalSessionRepository {
  ...
  public List<DigitalSession> findAllDigitalSession() {
    return jpaDigitalSessionRepository.findAll().stream().map(DigitalSessionEntity::toDomain)
  ...
  public List<DigitalSession> findDigitalSessionByUser(Long userId) {
    return jpaDigitalSessionRepository.findDigitalSessionByUser(userId).stream().map(DigitalSessionEntity::toDomain)
        .collect(Collectors.toList());
  }

  @Override
  public Long createDigitalSession(DigitalSession digitalSession) {
    UserEntity userEntity = jpaUserRepository.findById(digitalSession.getUserId())
  ...
  public Long updateDigitalSession(DigitalSession digitalSession) {
    DigitalSessionEntity digitalSessionEntity1 = jpaDigitalSessionRepository.findById(digitalSession.getId())
        .orElseThrow(IllegalArgumentException::new);
