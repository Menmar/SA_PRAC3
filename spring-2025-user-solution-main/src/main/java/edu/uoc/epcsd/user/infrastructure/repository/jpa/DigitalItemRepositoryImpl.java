...
public class DigitalItemRepositoryImpl implements DigitalItemRepository {
  ...
  public List<DigitalItem> findDigitalItemBySession(Long digitalSessionId) {
    return jpaDigitalItemRepository.findDigitalItemByDigitalSession(digitalSessionId).stream()
        .map(DigitalItemEntity::toDomain).collect(Collectors.toList());
  }

  @Override
  public Long createDigitalItem(DigitalItem digitalItem) {
    DigitalSessionEntity digitalSessionEntity = jpaDigitalSessionRepository.findById(digitalItem.getDigitalSessionId())
  ...
  public Long updateDigitalItem(DigitalItem digitalItem) {
    DigitalItemEntity digitalItemEntity = jpaDigitalItemRepository.findById(digitalItem.getId())
        .orElseThrow(IllegalArgumentException::new);
